...
export default function ThroughputChart() {
  ...
  const { data, isLoading } = useQuery<ThroughputResponse>({
    queryKey: ['throughput', range],
    queryFn: () =>
      api.get<ThroughputResponse>(`/events/throughput?range=${range}`).then((r) => r.data),
    ...
    <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-gray-900">Event throughput</h2>
          {!isLoading && (
            <span className="text-xs text-gray-400 font-normal">
              {totalInRange.toLocaleString()} events
            </span>
          )}
        </div>
      ...
      {isLoading ? (
        ...
        <div className="h-48 flex items-center justify-center text-sm text-gray-400">
          No events in the last {RANGE_LABELS[range]}
        </div>
      ) : (
