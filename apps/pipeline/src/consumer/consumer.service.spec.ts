...
describe('ConsumerService', () => {
  ...
  it('runs happy path: creates execution, fans out, marks processed, acks', async () => {
    ...
    expect(fanout.publishToDestination).toHaveBeenCalledWith('exec-1', 'dest-1', WORKSPACE_ID, 'Test Pipeline', expect.objectContaining({ eventId: EVENT_ID }))
  ...
  it('fans out to multiple destinations in a pipeline', async () => {
    ...
    expect(fanout.publishToDestination).toHaveBeenCalledWith('exec-1', 'dest-slack', WORKSPACE_ID, 'Test Pipeline', expect.anything())
    expect(fanout.publishToDestination).toHaveBeenCalledWith('exec-1', 'dest-s3', WORKSPACE_ID, 'Test Pipeline', expect.anything())
  })
})
